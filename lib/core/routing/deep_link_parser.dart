...
final class DeepLinkParser {
  ...
  String? _routeFromSegments(List<String> segments) {
    try {
      return switch (segments) {
        ...
        ['groups', final groupId, 'tasks', final taskId] => AppRoutes.taskDetail(groupId, taskId),
        ['groups', final groupId, 'alarms', final alarmId] => AppRoutes.alarmReceived(groupId, alarmId),
        ['groups', final groupId, 'alarms', final alarmId, 'received'] => AppRoutes.alarmReceived(groupId, alarmId),
        _ => null,
      };
    } on ArgumentError {
      return null;
  ...
  String? _routeFromType(Map<String, String> data) {
    try {
      return switch (data['type']) {
        'daily_digest' || 'queue' => AppRoutes.queue(data['queueId'] ?? 'recently-saved'),
        'group_invite' || 'group_activity' => _groupRoute(data),
        'task_assigned' || 'task_updated' || 'task_completed' => _taskRoute(data),
  ...
  String? _taskRoute(Map<String, String> data) {
    ...
    return groupId == null || taskId == null ? null : AppRoutes.taskDetail(groupId, taskId);
  }

  String? _alarmRoute(Map<String, String> data) {
    final groupId = data['groupId'];
    final alarmId = data['alarmId'];
    return groupId == null || alarmId == null ? null : AppRoutes.alarmReceived(groupId, alarmId);
  }
}
